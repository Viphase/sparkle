...
Keep this file short. The v2 docs carry the detail.
...
The recommended sequence (see roadmap.md for full criteria):
...
Reproduced from v2-vision.md so it's never out of sight:

- First-run wizard takes a cold user from `sparkle` to "I have a spark
  and an AI question to answer" in under a minute.
- The same app renders coherently on 60×20, 100×30, 120×36, and
  220×60 terminals.
- Pulse uses ntcharts (BarChart, Heatmap, Sparkline) — not unicode
  bars.
- Workspace shows rail + detail + AI side-by-side at width ≥ 120.
- Settings has an API-key field and a working Test connection button
  that round-trips an Anthropic Ping.
- Custom skills authored in `.sparkle/skills/` show in the picker.
